...
def run():
    ...
    with TestClient(app) as client:
        ...
        # Create resident
        r = client.post("/api/residents/", json={"first_name": "Jane", "last_name": "Doe"}, headers=headers)
        ...
        # Issue certificate
        c = client.post("/api/certificates/", json={"certificate_type_code": "RES", "resident_id": resident["id"]}, headers=headers)
        assert c.status_code == 201, f"issue certificate failed: {c.status_code} {c.text}"
        data = c.json()
