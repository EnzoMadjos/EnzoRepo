...
def build_user_prompt(agent_name: str, user_prompt: str, upstream_context: str = '') -> str:
    folder = BASE / agent_name.replace(' ', '-').lower()
...
def run_agent(agent_name: str, model: str, provider: str,
              user_prompt: str, upstream_context: str = '',
    ...
    if simulate:
        text = f"[SIMULATE] {agent_name} | {model} | {provider}\nPrompt: {user[:150]}..."
    else:
    ...
    return {'agent': agent_name, 'model': model, 'provider': provider, 'response': text, 'usage': locals().get('usage')}


def orchestrate(start_agent: str, initial_model: str, provider: str,
                prompt: str, simulate: bool = False, follow_model: str = None) -> tuple:
    start_agent_key = start_agent.replace(' ', '-').lower()
    results = []

    # Step 1: call the entry agent using the initial model
    first = run_agent(start_agent_key, initial_model, provider, prompt, simulate=simulate)
    ...
    if start_agent_key == 'architect':
        ...
        for follow in ARCHITECT_CHAIN:
            ...
            result = run_agent(follow, fm, provider, follow_prompt,
                               upstream_context=arch_context, simulate=simulate)
...
def main():
    ...
    p.add_argument('--force-model', default=None,
    ...
    p.add_argument('--prompt',   required=True, help='Your Salesforce request or question')
    ...
    try:
        path, result = orchestrate(args.agent, initial_model, provider, args.prompt, args.simulate, follow_model=follow_model)
        ...
        if args.hybrid and not args.simulate:
            ...
            aggregated = '\n\n'.join([r.get('response') if isinstance(r.get('response'), str) else str(r.get('response')) for r in result['responses']])
            ...
            final = run_agent(args.agent.replace(' ', '-').lower(), final_model, provider, review_prompt, upstream_context=summary, simulate=args.simulate)
