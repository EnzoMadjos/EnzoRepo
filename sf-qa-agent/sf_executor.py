    ...
    for op in plan:
        ...
        try:
            ...
            if action == "warning":
                ...
                msg = op.get(
                    "message",
                    op.get("label", "Ambiguous step — review before running."),
                )
                summary.append(
                    {
                        "step": step_num,
                ...
                if not record_id:
                    raise ValueError(
                        "update action requires a 'record_id' field (e.g. '$step1.id')"
                    )
                ...
                if not record_id:
                    raise ValueError(
                        "delete action requires a 'record_id' field (e.g. '$step1.id')"
                    )
                ...
                if not source_id:
                    raise ValueError(
                        "clone action requires a 'record_id' field (e.g. '$step1.id')"
                    )
